...
export function WireframeGlobe() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      projected.forEach((p) => {
        const opacity = p.z < 0 ? 0.22 : 0.05;
        ctx.fillStyle = `rgba(255, 140, 0, ${opacity})`;
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.z < 0 ? 1.6 : 0.9, 0, 2 * Math.PI);
        ctx.fill();
      });

      rotationY += 0.0008; // slow cinematic rotation
  ...
  return (
    <canvas
      ref={canvasRef}
      className="absolute right-0 top-1/2 -translate-y-[50%] w-[560px] h-[560px] pointer-events-none select-none opacity-25 lg:opacity-45 translate-x-[40%] z-0"
